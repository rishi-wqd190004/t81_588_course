...
def get_profits(profit_map):
  std_prof, holi_prof = 0.0,0.0
  for key in profit_map.keys():
    if 'holiday' in key:
      holi_prof += profit_map[key]
    else:
      std_prof += profit_map[key]
  return std_prof, holi_prof

last_year_standard_profit, last_year_holiday_profit = get_profits(profit_map)

for val in new_months_data:
  profit_map = profit_map.new_child(val)
